...
def sync():
    ...
    results = drive_service.files().list(q=f"'{settings.FOLDER_ID_CASH_OPS}' in parents and trashed=false", fields="files(id, name)").execute()
    files = results.get('files', [])

    if not files:
        print("No files in folder; nothing to sync.")
        return
    ...
    if combined_df.empty:
        crash_report("sync_cash", "Files found but no valid rows (Pre-Validation)", {"files": [x["name"] for x in files]}, None)
    ...
    if final_df.empty:
        crash_report("sync_cash", "Pre-Validation: 0 rows after clean; refusing WRITE_TRUNCATE", {"combined_rows": len(combined_df)}, None)
    ...
    job = bq_client.load_table_from_dataframe(final_df, f"{settings.PROJECT_ID}.{settings.DATASET_ID}.{settings.TABLE_CASH}", job_config=job_config)
...
if __name__ == "__main__":
    try:
        ...
        append_crash_state("sync_cash", str(e), getattr(e, 'state_of_data', {}))
        sys.exit(1)
